...
export const data = new SlashCommandBuilder()
  .setName('wipemembers')
  .setDescription(
    'Remove member role from all users in the server, except life members.',
  )
    ...
    if (index.customId === 'confirm') {
      ...
      await Promise.allSettled(
        guild.members.cache.map(async (member) => {
          if (
            member.roles.cache.has('753524901708693558') &&
            !member.roles.cache.has('702889882598506558')
          ) {
            ...
            if (removed % 10 === 0) {
              await interaction.editReply({
                content: `Removing members... ${removed} removed so far.`,
              })
            }
          }
        }),
