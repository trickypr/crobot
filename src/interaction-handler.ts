...
export default class InteractionHandler {
  ...
  public async showModal(
    modalDefinition: ModalBuilder,
    time = 60_000,
  ): Promise<ModalSubmitFields> {
    ...
    if (modalDefinition.data.custom_id === undefined) {
      modalDefinition.setCustomId(
        (modalDefinition.data.title ?? crypto.randomUUID())
          .toLowerCase()
          .replaceAll(/[^a-z]/g, ''),
      )
    }
    ...
    const modalInteraction = await this.interaction.awaitModalSubmit({
      ...
      filter: (filterInteraction) =>
        filterInteraction.user.id === this.interaction.user.id &&
        filterInteraction.customId === modalDefinition.data.custom_id &&
        !filterInteraction.replied,
    })
  ...
  public async showReplyWithActionRow(
    content: string,
    ...
    const message = await (this.interaction.replied
      ? this.interaction.editReply({ content, components: [actionRow] })
    ...
    const buttonInteraction = await message.awaitMessageComponent({
      ...
      filter: (filterInteraction) =>
        filterInteraction.user.id === this.interaction.user.id &&
        actionCallBacks.has(filterInteraction.customId) &&
        !filterInteraction.replied,
    })
  ...
  public async showReply(content: string): Promise<void> {
    await (this.interaction.replied
