...
async function main(): Promise<void> {
  ...
  const commands: Array<ReturnType<CommandDefinition['data']['toJSON']>> = []
  // Grab all the command files from the commands directory you created earlier
  let commandFiles = await fs.readdir(
  ...
  for (const file of commandFiles) {
    const command: CommandDefinition = await import(`./commands/${file}`)
  ...
  try {
    ...
    console.log(
      `Successfully reloaded ${(data as never[]).length} application (/) commands.`,
    )
  } catch (error) {
